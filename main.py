                ...
                if os.path.isdir(potential_path):
                    for root, _, files in os.walk(potential_path):
                        if "java.exe" in files and "bin" in root.lower():
                            found_paths.add(
                                os.path.normpath(os.path.join(root, "java.exe"))
                            )
...
def main(page: ft.Page):
    ...
    completion_sound = Audio(
        src="https://www.soundjay.com/buttons/sounds/button-3.mp3", autoplay=False
    )
    ...
    command_input = ft.TextField(
        label="输入服务器命令...", expand=True, border_radius=ft.border_radius.all(8)
    )

    server_status_text = ft.Text(
        "服务器状态: 未运行", color=ft.Colors.RED, weight=ft.FontWeight.BOLD
    )
    player_count_text = ft.Text("玩家: 0/20")

    start_button = ft.FilledButton(
        "启动服务器", icon=ft.Icons.PLAY_ARROW_ROUNDED, disabled=True
    )
    ...
    restart_button = ft.FilledButton(
        "重启服务器", icon=ft.Icons.RESTART_ALT_ROUNDED, disabled=True
    )
    configure_button = ft.FilledButton(
        "配置",
        icon=ft.Icons.EDIT_NOTE_ROUNDED,
        disabled=True,
        tooltip="编辑 server.properties",
    )
    ...
    def update_console_output():
        ...
        while server_process.poll() is None:
            try:
                ...
                if list_match:
                    ...
                    if player_names_str:
                        online_players.current = sorted(
                            [name.strip() for name in player_names_str.split(",")]
                        )
                    else:
                        online_players.current = []
                    page.update()
                ...
                join_match = re.search(
                    r"(\w+)\[.*logged in|\]: (\w+) joined the game", cleaned_line
                )
                if join_match:
        ...
        console_output.controls.append(
            create_console_text("服务器已停止。", color=ft.Colors.RED)
        )
        page.update()
    ...
    def start_server(e):
        ...
        if not selected_server_path.current:
            console_output.controls.append(
                create_console_text(
                    "错误: 请先选择一个服务器实例。", color=ft.Colors.RED
                )
            )
            page.update()
            return
        ...
        if server_process is None:
            ...
            if not jar_files:
                console_output.controls.append(
                    create_console_text(
                        f"错误: 在 '{os.path.basename(server_dir)}' 目录中未找到 .jar 文件。",
                        color=ft.Colors.RED,
                    )
                )
                page.update()
            ...
            console_output.controls.append(
                create_console_text(
                    f"正在启动服务器 '{os.path.basename(server_dir)}'...",
            ...
            try:
                ...
                console_output.controls.append(
                    create_console_text(
                        f"执行命令: {' '.join(command)}", color=ft.Colors.GREY
                    )
                )
                page.update()
                ...
                server_thread = threading.Thread(
                    target=update_console_output, daemon=True
                )
                server_thread.start()
                performance_thread = threading.Thread(
                    target=update_performance_stats, daemon=True
                )
                performance_thread.start()
                player_list_thread = threading.Thread(
                    target=update_player_list_periodically, daemon=True
                )
                player_list_thread.start()
                server_status_text.value = (
                    f"服务器状态: 运行中 ({os.path.basename(server_dir)})"
                )
                ...
                console_output.controls.append(
                    create_console_text(
                        "错误: 'java' 命令未找到。请确保已安装 Java 并将其添加至系统 PATH。",
                        color=ft.Colors.RED,
                    )
                )
            except Exception as ex:
                console_output.controls.append(
                    create_console_text(f"启动失败: {ex}", color=ft.Colors.RED)
                )
            page.update()
    ...
    def send_command(e):
        if server_process and server_process.stdin and command_input.value:
            try:
                ...
                console_output.controls.append(
                    create_console_text(
                        f"> {command_input.value}", color=ft.Colors.CYAN
                    )
                )
                command_input.value = ""
            except Exception as ex:
                console_output.controls.append(
                    create_console_text(f"命令发送失败: {ex}", color=ft.Colors.RED)
                )
            page.update()
    ...
    def stop_server_action():
        if server_process and server_process.stdin:
            console_output.controls.append(
                create_console_text("正在停止服务器...", color=ft.Colors.ORANGE)
            )
            page.update()
            try:
    ...
    def restart_server(e):
        console_output.controls.append(
            create_console_text("正在重启服务器...", color=ft.Colors.BLUE)
        )
        page.update()
        stop_server_action()
    ...
    send_button = ft.IconButton(
        icon=ft.Icons.SEND_ROUNDED, on_click=send_command, tooltip="发送命令"
    )
    ...
    def create_home_view():
        def copy_console_output(e):
            console_texts = [
                c.value
                for c in console_output.controls
                if isinstance(c, ft.Text) and c.value is not None
            ]
            all_text = "\n".join(console_texts)
            page.set_clipboard(all_text)
            page.overlay.append(
                ft.SnackBar(ft.Text("控制台内容已复制到剪贴板。"), open=True)
            )
            page.update()
        ...
        def open_properties_editor(e):
            if not selected_server_path.current:
                page.overlay.append(
                    ft.SnackBar(ft.Text("请先选择一个服务器!"), open=True)
                )
                page.update()
                return

            properties_path = os.path.join(
                selected_server_path.current, "server.properties"
            )
            if not os.path.exists(properties_path):
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text(
                            f"在 '{os.path.basename(selected_server_path.current)}' 中未找到 server.properties 文件。"
                        ),
                        open=True,
                    )
                )
            ...
            try:
                ...
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"读取配置失败: {ex}"), open=True)
                )
                page.update()
                return

            property_fields = [
                ft.TextField(label=key, value=value, data=key, dense=True)
            ...
            def save_properties(e_save):
                ...
                try:
                    ...
                    dialog.open = False
                    page.overlay.append(
                        ft.SnackBar(ft.Text("配置已成功保存!"), open=True)
                    )
                    page.update()
                except Exception as ex:
                    page.overlay.append(
                        ft.SnackBar(ft.Text(f"保存配置失败: {ex}"), open=True)
                    )
                    page.update()
            ...
            dialog.title = ft.Text(
                f"编辑 {os.path.basename(selected_server_path.current)}/server.properties"
            )
        ...
        def delete_server_action(e):
            ...
            if not path_to_delete or not os.path.exists(path_to_delete):
                page.overlay.append(
                    ft.SnackBar(ft.Text("错误: 服务器路径无效或已不存在。"), open=True)
                )
                dialog.open = False
                page.update()
                return
            ...
            try:
                ...
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text(
                            f"服务器 '{os.path.basename(path_to_delete)}' 已被删除。"
                        ),
                        open=True,
                    )
                )
        ...
        def on_server_selected(e):
            ...
            if server_name:
                selected_server_path.current = os.path.join(
                    SERVERS_ROOT_DIR, server_name
                )
                ...
                console_output.controls.append(
                    create_console_text(
                        f"已选择服务器: {server_name}", color=ft.Colors.BLUE
                    )
                )
            else:
        ...
        def update_server_list(e=None):
            try:
                server_dirs = [
                    d
                    for d in os.listdir(SERVERS_ROOT_DIR)
                    if os.path.isdir(os.path.join(SERVERS_ROOT_DIR, d))
                ]
                server_selector_dropdown.options = [
                    ft.dropdown.Option(d) for d in server_dirs
                ]

                current_selection = (
                    os.path.basename(selected_server_path.current)
                    if selected_server_path.current
                    else None
                )
                ...
                if e:
                    page.overlay.append(
                        ft.SnackBar(
                            ft.Text("服务器列表已刷新"), duration=2000, open=True
                        )
                    )

            except Exception as ex:
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"刷新列表失败: {ex}"), open=True)
                )
            page.update()
    ...
    def create_settings_view():
        ...
        def color_option_clicked(e):
            ...
            for control in color_options.controls:
                if isinstance(control, ft.Container):
                    control.border = (
                        ft.border.all(3, ft.Colors.OUTLINE)
                        if control.data == selected_color.current
                        else None
                    )
        ...
        color_options = ft.GridView(
            expand=False, max_extent=50, spacing=10, run_spacing=10, height=120
        )
        for color_name, color_value in PREDEFINED_COLORS.items():
            color_options.controls.append(
                ft.Container(
                    ...
                    if color_value == selected_color.current
                    else None,
                )
            )
        ...
        def auto_detect_java(e):
            ...
            if not found_paths:
                page.overlay.append(
                    ft.SnackBar(ft.Text("未在标准目录中找到 Java。"), open=True)
                )
                page.update()
                return
            ...
            if len(found_paths) == 1:
                ...
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"成功找到 Java: {found_paths[0]}"), open=True)
                )
                page.update()
                return
            ...
            java_selection_dialog = ft.AlertDialog(
                ...
                content=ft.Container(
                    ft.RadioGroup(
                        ...
                        value=java_path_field.value
                        or found_paths[0],  # Set initial selection
                        content=ft.Column(
        ...
        def save_app_settings(e):
            ...
            app_settings["primary_color"] = (
                selected_color.current or ft.Colors.BLUE_GREY
            )
            ...
            app_settings["download_source"] = (
                download_source_dropdown.value or "Official"
            )
            save_settings()
            ...
            page.dark_theme = ft.Theme(
                color_scheme_seed=primary_color, font_family="Roboto"
            )

            page.overlay.append(
                ft.SnackBar(
                    ft.Text("设置已保存! 部分主题更改可能需要重启应用。"), open=True
                )
            )
            page.update()

        save_button = ft.FilledButton(
            "保存设置", icon=ft.Icons.SAVE_ROUNDED, on_click=save_app_settings
        )
        ...
        return ft.Column(
            [
                ...
                SettingsCard("网络设置", [download_source_dropdown]),
                save_button,
            ],
            spacing=10,
            expand=True,
    ...
    def create_player_management_view():
        ...
        def get_json_list_path(list_type: str) -> Optional[str]:
            ...
            return os.path.join(
                selected_server_path.current, filename_map.get(list_type, "")
            )
        ...
        def save_json_list(list_type: str, data: List[dict[str, Any]]):
            ...
            try:
                ...
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"保存文件失败: {e}"), open=True)
                )
                page.update()
            ...
            def execute_player_command(command: str, player: str):
                ...
                if server_process and server_process.stdin:
                    try:
                        ...
                        console_output.controls.append(
                            create_console_text(
                                f"> {full_command}", color=ft.Colors.CYAN
                            )
                        )
                        page.overlay.append(
                            ft.SnackBar(
                                ft.Text(f"命令 '{full_command}' 已发送。"), open=True
                            )
                        )
                    except Exception as ex:
                        page.overlay.append(
                            ft.SnackBar(ft.Text(f"命令发送失败: {ex}"), open=True)
                        )
                else:
                    page.overlay.append(
                        ft.SnackBar(ft.Text("服务器未运行或无法发送命令。"), open=True)
                    )
                page.update()
                    ...
                    for player_name in online_players.current:
                        player_list_view.controls.append(
                            ft.ListTile(
                                ...
                                title=ft.Text(player_name, weight=ft.FontWeight.BOLD),
                                trailing=ft.Row(
                                    [
                                        ft.IconButton(
                                            icon=ft.Icons.DO_NOT_DISTURB_ON,
                                            tooltip="踢出",
                                            on_click=lambda _,
                                            p=player_name: execute_player_command(
                                                "kick", p
                                            ),
            ...
            online_tab_content = ft.Column(
                [
                    ft.Row(
                        [
                            ft.Text(
                                "实时在线玩家", style=ft.TextThemeStyle.TITLE_MEDIUM
                            ),
                            ft.IconButton(
                                icon=ft.Icons.REFRESH,
                                on_click=refresh_online_list,
                                tooltip="刷新列表",
                            ),
                        ],
        ...
        def create_json_list_tab(list_type: str, title: str):
            ...
            def add_player_thread(name: str):
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text(f"正在查找玩家 '{name}'..."), open=True, duration=4000
                    )
                )
                page.update()
                ...
                if not player_data:
                    page.overlay.append(
                        ft.SnackBar(
                            ft.Text(f"玩家 '{name}' 未找到或 Mojang API 出错。"),
                            open=True,
                        )
                    )
                ...
                if any(p.get("uuid", "").replace("-", "") == player_uuid for p in data):
                    page.overlay.append(
                        ft.SnackBar(
                            ft.Text(f"玩家 '{corrected_name}' 已在列表中。"), open=True
                        )
                    )
                    add_player_button.disabled = False
                    page.update()
                ...
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text(
                            f"玩家 '{corrected_name}' 已添加。服务器可能需要重载才能生效。"
                        ),
                        open=True,
                    )
                )
            ...
            def remove_player(e):
                ...
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text("玩家已移除。服务器可能需要重载才能生效。"), open=True
                    )
                )
                page.update()
        ...
        def on_tab_change(e):
            ...
            if hasattr(selected_tab_content, "data") and callable(
        ...
        def refresh_all_views():
            ...
            on_tab_change(
                ft.ControlEvent(
                    target=tabs_control.uid,
                    name="change",
                    data=str(tabs_control.selected_index),
                    control=tabs_control,
                    page=page,
                )
            )

        view_container = ft.Column(
            [ft.Text("玩家管理", style=ft.TextThemeStyle.HEADLINE_SMALL), tabs_control],
            expand=True,
        )
    ...
    def create_file_manager_view():
        ...
        def save_file(e):
            if not current_editing_path.current:
                return
            try:
                with open(current_editing_path.current, "w", encoding="utf-8") as f:
                    f.write(editor_textfield.value or "")
                edit_dialog.open = False
                page.overlay.append(
                    ft.SnackBar(
                        ft.Text(
                            f"文件 '{os.path.basename(current_editing_path.current)}' 已保存!"
                        ),
                        open=True,
                    )
                )
        ...
        def open_editor(file_path):
            try:
                if os.path.getsize(file_path) > 5 * 1024 * 1024:
                    page.overlay.append(
                        ft.SnackBar(
                            ft.Text("文件太大 (>5MB)，无法在应用内编辑。"), open=True
                        )
                    )
                    page.update()
                    return
                ...
                edit_dialog.title = ft.Text(
                    f"编辑: {os.path.relpath(file_path, base_path)}"
                )
                edit_dialog.content = ft.Container(
                    editor_textfield, width=800, height=600, padding=5
                )
                edit_dialog.actions = [
                ...
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"无法打开或读取文件: {e}"), open=True)
                )
                page.update()
        ...
        def show_item_details(item_path):
            ...
            try:
                ...
                if is_dir:
                    ...
                    details_controls_list.append(
                        ft.Text("内容预览:", weight=ft.FontWeight.BOLD)
                    )
                    try:
                        ...
                        if not items:
                            ...
                            for i, item_name in enumerate(items[:15]):
                                icon = (
                                    ...
                                    if os.path.isdir(os.path.join(item_path, item_name))
                                    else ft.Icons.DESCRIPTION_OUTLINED
                                )
                                content_preview.controls.append(
                                    ft.Row([ft.Icon(icon, size=16), ft.Text(item_name)])
                                )
                            if len(items) > 15:
                        ...
                        details_controls_list.append(
                            ft.Text(f"无法预览内容: {preview_e}", color=ft.Colors.RED)
                        )
                    ...
                    details_controls_list.insert(
                        2, ft.Text(f"大小: {size_str}", selectable=True)
                    )
                    editable_extensions = [
                        ".txt",
                        ".yml",
                ...
                file_details_view.controls.append(
                    ft.Text(f"无法读取详情: {e}", color=ft.Colors.RED)
                )
            page.update()
        ...
        def list_directory(path):
            current_path_text.value = (
                f"当前: {os.path.relpath(path, os.path.dirname(base_path))}"
            )
            file_list_view.controls.clear()
            file_details_view.controls.clear()

            def create_list_item(
                text, icon, on_click_handler, data=None, is_dir=False, weight=None
            ):
                return ft.Container(
                    content=ft.ListTile(
                        title=ft.Text(text, weight=weight),
            ...
            if os.path.abspath(path) != base_path:
                file_list_view.controls.append(
                    create_list_item(
                        ...
                        lambda e: list_directory(os.path.dirname(path)),
                    )
                )
            ...
            try:
                ...
                for item in os.listdir(path):
                    (dirs if os.path.isdir(os.path.join(path, item)) else files).append(
                        item
                    )
                ...
                file_list_view.controls.append(
                    ft.Text(f"无法访问目录: {e}", color=ft.Colors.RED)
                )
            page.update()
                    ...
                    [
                        ft.Container(
                            ft.Column(
                                [
                                    ft.Text(
                                        "文件列表", style=ft.TextThemeStyle.TITLE_MEDIUM
                                    ),
                                    ft.Divider(height=1),
                                    ft.Container(
    ...
    def create_plugin_manager_view():
        ...
        plugin_details_view = ft.Column(
            visible=False, spacing=10, scroll=ft.ScrollMode.ADAPTIVE
        )
        plugin_versions_dropdown = ft.Dropdown(label="选择插件版本", expand=True)
        download_button = ft.FilledButton(
            "下载到服务器", icon=ft.Icons.DOWNLOAD, disabled=True
        )
        ...
        def update_installed_plugins_list(e=None):
            ...
            if not selected_server_path.current:
                ...
                plugin_files = [
                    f for f in os.listdir(plugins_dir) if f.endswith(".jar")
                ]
                if not plugin_files:
                    installed_plugins_list.controls.append(
                        ft.Text("没有已安装的插件。")
                    ...
                    for plugin_file in plugin_files:
                        installed_plugins_list.controls.append(
                            ft.ListTile(
                                leading=ft.Icon(ft.Icons.EXTENSION),
                                title=ft.Text(plugin_file),
                            )
                        )
            page.update()
        ...
        def download_plugin_thread(version_data: dict[str, Any]):
            if not selected_server_path.current:
                page.overlay.append(
                    ft.SnackBar(ft.Text("错误: 未选择服务器。"), open=True)
                )
                page.update()
                return
            try:
                ...
                with requests.get(
                    download_url, stream=True, timeout=300, headers=REQUESTS_HEADERS
                ) as r:
                    r.raise_for_status()
                ...
                page.overlay.append(
                    ft.SnackBar(ft.Text(f"插件 '{filename}' 下载成功!"), open=True)
                )
        ...
        def on_download_click(e):
            if plugin_versions_dropdown.value:
                ...
                page.run_thread(
                    download_plugin_thread, json.loads(plugin_versions_dropdown.value)
                )

        def fetch_plugin_versions_thread_modrinth(
            project_id: str, status_text_control: ft.Text
        ):
            ...
            try:
                ...
                if not versions:
                    if server_game_version:
                        status_text_control.value = (
                            f"未找到适用于 Minecraft {server_game_version} 的版本。"
                        )
                    else:
                ...
                for v in versions:
                    primary_file = next(
                        (f for f in v["files"] if f["primary"]), v["files"][0]
                    )
                    ...
                    plugin_versions_dropdown.options.append(
                        ft.dropdown.Option(
                            key=option_data, text=f"{v['name']} ({v['version_number']})"
                        )
                    )
        ...
        def fetch_plugin_versions_thread_papermc(
            project_data: dict, status_text_control: ft.Text
        ):
            plugin_versions_dropdown.options = []
            plugin_versions_dropdown.value = None
            plugin_versions_dropdown.disabled = True
        ...
        def on_search_result_click(e):
            ...
            plugin_details_view.controls = [
                ...
                ft.Text(
                    project["author"], style=ft.TextThemeStyle.BODY_SMALL, italic=True
                ),
            ...
            if source == "modrinth":
                page.run_thread(
                    fetch_plugin_versions_thread_modrinth,
                    project["project_id"],
                    version_filter_status,
                )
            elif source == "papermc":
                page.run_thread(
                    fetch_plugin_versions_thread_papermc, project, version_filter_status
                )
        ...
        def search_modrinth_thread(query: str):
            ...
            try:
                ...
                search_results_list.controls.clear()
                search_results_list.controls.append(
                    ft.Text(f"搜索失败: {type(ex).__name__}: {ex}", color=ft.Colors.RED)
                )
            page.update()
                ...
                for hit in data.get("result", []):
                    ...
                    search_results_list.controls.append(
                        ft.ListTile(
                            ...
                            title=ft.Text(
                                project_data["title"], weight=ft.FontWeight.BOLD
                            ),
                            subtitle=ft.Text(
                            ...
                            [
                                ft.Row(
                                    [
                                        ...
                                        ft.IconButton(
                                            icon=ft.Icons.REFRESH,
                                            on_click=update_installed_plugins_list,
                                        ),
                                    ]
                                ),
    ...
    def create_core_download_view():
        ...
        server_name_input = ft.TextField(
            label="为新服务器命名", expand=True, autofocus=True
        )
        create_server_button = ft.FilledButton(
            "创建并设置核心", icon=ft.Icons.CREATE_NEW_FOLDER
        )
        ...
        download_button = ft.FilledButton(
            "下载核心", icon=ft.Icons.DOWNLOAD, disabled=True
        )
        ...
        def show_version_list(core_type):
            ...
            try:
                if core_type == "Paper":
                    r = requests.get(
                        f"{get_api_base_url('paper')}/v3/projects/paper",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                    ...
                    r = requests.get(
                        f"{get_api_base_url('purpur')}/v2/purpur",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                    ...
                    r = requests.get(
                        f"{get_api_base_url('getbukkit_page')}/download/spigot",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                    versions = re.findall(
                        r'<h2><a href=".+?">Spigot ([0-9\.]+)</a></h2>', r.text
                    )
                    if not versions:
                        versions = re.findall(
                            r'href=".+?/spigot-([0-9\.]+)\.jar"', r.text
                        )
                    ...
                    r = requests.get(
                        f"{get_api_base_url('mojang_meta')}/mc/game/version_manifest.json",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
        ...
        def show_build_list(core_type, version):
            ...
            def add_build_tile(build):
                build_list_view.controls.append(
                    ft.ListTile(
                        ...
                        on_click=lambda e, b=build: enable_download(
                            core_type, version, b
                        ),
                    )
                )
            ...
            try:
                if core_type == "Paper":
                    r = requests.get(
                        f"{get_api_base_url('paper')}/v3/projects/paper/versions/{version}",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                    ...
                    r = requests.get(
                        f"{get_api_base_url('purpur')}/v2/purpur/{version}",
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                    ...
                    build_list_view.controls.append(
                        ft.Text("无需选择构建号，直接下载。", color=ft.Colors.GREY)
                    )
        ...
        def enable_download(core_type, version, build):
            ...
            update_status(
                f"准备下载 {core_type} {version} {build if build else ''}",
                ft.Colors.BLUE,
            )
            page.update()
        ...
        def download_core_thread(core_type, version, build):
            ...
            try:
                if core_type == "Paper":
                    ...
                    r = requests.get(
                        core_api_data["vanilla_versions"][version],
                        timeout=REQUESTS_TIMEOUT,
                        headers=REQUESTS_HEADERS,
                    )
                    r.raise_for_status()
                ...
                with requests.get(
                    url, stream=True, timeout=300, headers=REQUESTS_HEADERS
                ) as r:
                    r.raise_for_status()
                    total_size = int(r.headers.get("content-length", 0))
                    bytes_downloaded = 0
                    with open(final_path, "wb") as f:
                        for chunk in r.iter_content(chunk_size=8192):
                            ...
                            if total_size > 0:
                                ...
                                update_status(
                                    f"下载中... {bytes_downloaded // 1024} KB / {total_size // 1024} KB",
                                    ft.Colors.BLUE,
                                    True,
                                )
                try:
                    ...
                    with open(eula_path, "w") as f:
                        f.write("eula=true\n")
                    update_status(
                        f"下载完成! 已保存到: {final_path} 并自动同意 EULA。",
                        ft.Colors.GREEN,
                    )
                except Exception as eula_e:
                    update_status(
                        f"下载完成，但自动同意 EULA 失败: {eula_e}", ft.Colors.ORANGE
                    )
                completion_sound.play()
            except Exception as e:
        ...
        view = ft.Column(
            [
                ...
                SettingsCard(
                    "1. 创建新服务器",
                    [
                        ft.Row([server_name_input, create_server_button], spacing=10),
                        ft.Text(
                            "服务器文件将保存在 'servers/您输入的名字/' 文件夹中。"
                        ),
                    ],
                ),
                ...
                SettingsCard(
                    "5. 下载",
                    [download_button, download_status_text, download_progress],
                ),
            ],
            expand=True,
            spacing=10,
    ...
    def init_navigation():
        ...
        def switch_view(index):
            ...
            if hasattr(current_view.current, "data") and callable(
